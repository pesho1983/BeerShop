
<!doctype html>
<html lang="en">
    <head>
        <link type="text/css" rel="stylesheet" media="screen" href="https://netdna.bootstrapcdn.com/font-awesome/4.1.0/css/font-awesome.min.css">
        <link rel="stylesheet" href="../../../Vendor/css/styles.css">
    </head>
    
    <body>
    <header>
        <h1>Quality House Beer</h1>
        <nav>
        <a href="http://localhost/Beershop/QH_Beer/Templates/homepage/index.php" class="navbar-left"><img src="../../../Vendor/images/logoNew_bubbles.png"></a>
            <ul>
                <li><a href="http://localhost/Beershop/QH_Beer/Templates/homepage/index.php"><i class="fa fa-home"></i> HOME</a></li>
                <li><a href="http://localhost/Beershop/QH_Beer/Templates/catalog/catalog.php"><i class="fa fa-list"></i> CATALOG</a></li>
                <li><a href="http://localhost/Beershop/QH_Beer/Templates/beer/about.php"><i class="fa fa-pencil"></i> ABOUT</a></li>
                <li><a href="http://localhost/Beershop/QH_Beer/Templates/users/register.php"><i class="fa fa-user"></i> REGISTER</a></li>
                <li><a href="http://localhost/Beershop/QH_Beer/Templates/users/login.php"><i class="fa fa-user"></i>LOGIN</a></li>
                <li><a href="http://localhost/Beershop/QH_Beer/Templates/basket/basket.php"><i class="fa fa-beer"></i> BASKET</a></li>
                <li><a href="http://localhost/Beershop/QH_Beer/Templates/faq/faq.php"><i class="fa fa-question"></i> FAQ</a></li>
                <li><a href="#"><i class="fa fa-search"></i><input style="box-sizing: border-box; border: 2px solid; border-radius: 15px;" type="text" placeholder=" Search..." name="search"></a></li>
            </ul>
        </nav>
    </header>
    
    </body>
</html>